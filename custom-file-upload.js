...
function processFile() {
    ...
    if (!fileError) {
        if (KDF.getVal('txt_filename_one') === '') {
            ...
            fileError = false;
        } else if (KDF.getVal('txt_filename_three') === '') {
            fileError = false;
        } else if (KDF.getVal('txt_filename_four') === '') {
            fileError = false;
        } else if (KDF.getVal('txt_filename_five') === '') {
            fileError = false;
        } else {
            fileError = true;
            KDF.showError('Maximum file upload has been reached');
        }
    ...
    if (!fileError) {
        if (KDF.getVal('txt_filename_one') == fileName) {
            ...
            fileError = true;
        } else if (KDF.getVal('txt_filename_three') == fileName) {
            fileError = true;
        } else if (KDF.getVal('txt_filename_four') == fileName) {
            fileError = true;
        } else if (KDF.getVal('txt_filename_five') == fileName) {
            fileError = true;
        } else {
            fileError = false;
        }
    ...
    if (action === 'sharepoint_token') {
        ...
        if (!KDF.kdf().form.readonly && formParams.deleteFileSelector == '') {
            ...
            if (KDF.kdf().viewmode == 'U' && formParams.fileBlob == '') {
                ...
                if (KDF.getVal('txt_filename_three') !== '') {
                    sharepointFileThumbnail(KDF.getVal('txt_sharepointID_three'), access_token, 'txt_filename_three')
                }
                if (KDF.getVal('txt_filename_four') !== '') {
                    sharepointFileThumbnail(KDF.getVal('txt_sharepointID_four'), access_token, 'txt_filename_four')
                }
                if (KDF.getVal('txt_filename_five') !== '') {
                    sharepointFileThumbnail(KDF.getVal('txt_sharepointID_five'), access_token, 'txt_filename_five')
                }
        ...
        if (response.data['pass_status']) {
            ...
            if ($('#dform_widget_txt_sharepoint_title').length > 0) {
                ...
                sharepoint_title = 'Please upload up to five photos of the problem';
                console.log('123')
            }
...
function sharepointFileUploader(access_token) {
    ...
    $.ajax({
        ...
        } else if (KDF.getVal('txt_sharepointID_two') == '') {
        ...
        } else if (KDF.getVal('txt_sharepointID_three') == '') {
            KDF.setVal('txt_sharepointID_three', response.id);
            KDF.setVal('txt_filename_three', fileName);
            KDF.setVal('txt_sharepoint_link_three', response['webUrl']);
        } else if (KDF.getVal('txt_sharepointID_four') == '') {
            KDF.setVal('txt_sharepointID_four', response.id);
            KDF.setVal('txt_filename_four', fileName);
            KDF.setVal('txt_sharepoint_link_four', response['webUrl']);
        } else if (KDF.getVal('txt_sharepointID_five') == '') {
            KDF.setVal('txt_sharepointID_five', response.id);
            KDF.setVal('txt_filename_five', fileName);
            KDF.setVal('txt_sharepoint_link_five', response['webUrl']);
        }
    ...
    $.ajax({
        ...
        if (!KDF.kdf().form.readonly) {
            ...
            if (KDF.kdf().viewmode === 'U' && formParams.fileBlob == '') {
                ...
                } else if (widgetName == 'txt_filename_three') {
                    KDF.setVal('txt_filename_three_thumb', response.value[0].medium['url']);
                } else if (widgetName == 'txt_filename_four') {
                    KDF.setVal('txt_filename_four_thumb', response.value[0].medium['url']);
                } else if (widgetName == 'txt_filename_five') {
                    KDF.setVal('txt_filename_five_thumb', response.value[0].medium['url']);
                }
                ...
                if (KDF.getVal('txt_filename_one_thumb') == '') {
                    KDF.setVal('txt_filename_one_thumb', response.value[0].medium['url']);
                } else if (KDF.getVal('txt_filename_two_thumb') == '') {
                    KDF.setVal('txt_filename_two_thumb', response.value[0].medium['url']);
                } else if (KDF.getVal('txt_filename_three_thumb') == '') {
                    KDF.setVal('txt_filename_three_thumb', response.value[0].medium['url']);
                } else if (KDF.getVal('txt_filename_four_thumb') == '') {
                    KDF.setVal('txt_filename_four_thumb', response.value[0].medium['url']);
                } else if (KDF.getVal('txt_filename_five_thumb') == '') {
                    KDF.setVal('txt_filename_five_thumb', response.value[0].medium['url']);
                }

                $(".dform_fileupload_progressbar").html("<div style='width: 100%;'>");
                setTimeout(function() { addFileContainer();
                    $(".dform_fileupload_progressbar").html("<div style='width: 0%;'>"); }, 1000);
            }
...
function addFileContainer(widgetName) {
    ...
    if (KDF.kdf().viewmode == 'U' && formParams.fileBlob == '') {
        ...
        } else if ($('.filenames .txt_filename_three').length < 1) {
            fileName = KDF.getVal('txt_filename_three');
            fileThumbnail = KDF.getVal('txt_filename_three_thumb');
            widgetName = 'txt_filename_three';

        } else if ($('.filenames .txt_filename_four').length < 1) {
            fileName = KDF.getVal('txt_filename_four');
            fileThumbnail = KDF.getVal('txt_filename_four_thumb');
            widgetName = 'txt_filename_four';

        } else if ($('.filenames .txt_filename_five').length < 1) {
            fileName = KDF.getVal('txt_filename_five');
            fileThumbnail = KDF.getVal('txt_filename_five_thumb');
            widgetName = 'txt_filename_five';
        }
    }
...
function sharepointDownloadFile(access_token) {
    ...
    } else if (selector === 'txt_filename_two') {
        sharepointID = KDF.getVal('txt_sharepointID_two');
    } else if (selector === 'txt_filename_three') {
        sharepointID = KDF.getVal('txt_sharepointID_three');
    } else if (selector === 'txt_filename_four') {
        sharepointID = KDF.getVal('txt_sharepointID_four');
    } else if (selector === 'txt_filename_five') {
        sharepointID = KDF.getVal('txt_sharepointID_five');
    }
...
function deleteFile(access_token) {
    ...
    } else if (formParams.deleteFileSelector.includes('three')) {
        fileID = KDF.getVal('txt_sharepointID_three')
    } else if (formParams.deleteFileSelector.includes('four')) {
        fileID = KDF.getVal('txt_sharepointID_four')
    } else if (formParams.deleteFileSelector.includes('five')) {
        fileID = KDF.getVal('txt_sharepointID_five')
    }
    ...
    $.ajax({
        ...
        } else if (selector.includes('three')) {
            $('span.txt_filename_three').remove();
            KDF.setVal('txt_sharepointID_three', '')
            KDF.setVal('txt_filename_three', '')
            KDF.setVal('txt_filename_three_thumb', '')
        } else if (selector.includes('four')) {
            $('span.txt_filename_four').remove();
            KDF.setVal('txt_sharepointID_four', '')
            KDF.setVal('txt_filename_four', '')
            KDF.setVal('txt_filename_four_thumb', '')
        } else if (selector.includes('five')) {
            $('span.txt_filename_five').remove();
            KDF.setVal('txt_sharepointID_five', '')
            KDF.setVal('txt_filename_five', '')
            KDF.setVal('txt_filename_five_thumb', '')
        }
